module App
  module Charts
    class Friends < Base
      ...
      def regenerate!
        friends = self.account.friends.recent.group_by { |p| p.timestamp.beginning_of_month.to_s(:month) }
        
        values = []
        
        friends.each do |month, collection|
          values << { :value => collection.size, :label => month }
        end
        
        chart = build_line_chart(values, "#val# people", "friends count")
