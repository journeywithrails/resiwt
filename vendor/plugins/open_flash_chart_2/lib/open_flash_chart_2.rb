module OFC2
    ...
    <<-EOF
      ...
      <script type="text/javascript">
        ...
        swfobject.embedSWF(
          ...
          '#{width}', '#{height}','9.0.124', '#{swf_base}express_install.swf',
  ...
  def ofc2(width, height, url, base='/', id =Time.now.usec, swf_base='/', flash_attributes = {}, flash_params = {})
    ...
    <<-EOF
      ...
      <script type="text/javascript">
        swfobject.embedSWF(
        "#{swf_base}chart.swf","#{div_name}",
        "#{width}", "#{height}", "9.0.0", "/flash/express_install.swf",
        {"data-file":"#{base}#{url}"}, {#{flash_params.to_json if flash_params.present?}}, {#{flash_attributes.to_json if flash_attributes.present?}} );
      </script>
    EOF
  end
